...
class BillTrackerCard extends HTMLElement {
  ...
  _chart() {
    ...
    const grid = [0, 0.25, 0.5, 0.75, 1].map((ratio) => {
      ...
      return `<line x1="${left}" y1="${gy}" x2="${width - right}" y2="${gy}" class="grid" />
        <text x="${left - 8}" y="${gy + 4}" text-anchor="end" class="axis-value">${this._escape(this._compactMoney(val))}</text>`;
    }).join("");
  ...
  _render() {
    ...
    this.shadowRoot.getElementById("open-transfer")?.addEventListener("click", () => this._openTransfer());
    this.shadowRoot.getElementById("transfer-close")?.addEventListener("click", () => this._closeTransfer());
    this.shadowRoot.getElementById("transfer-modal")?.addEventListener("click", (event) => { if (event.target?.id === "transfer-modal") this._closeTransfer(); });
    this.shadowRoot.getElementById("transfer-modal")?.addEventListener("keydown", (event) => { if (event.key === "Escape") this._closeTransfer(); });
    this.shadowRoot.getElementById("import-csv-file")?.addEventListener("change", (event) => this._readImportFile(event.target));
    this.shadowRoot.getElementById("download-template")?.addEventListener("click", () => this._downloadTemplate());
    this.shadowRoot.getElementById("import-csv")?.addEventListener("click", () => this._importCsv());
    this.shadowRoot.getElementById("export-data")?.addEventListener("click", () => this._exportData());
    this.shadowRoot.getElementById("export-format")?.addEventListener("change", (event) => { this._exportFormat = event.target.value || "csv"; this._render(); });
    this.shadowRoot.getElementById("export-status")?.addEventListener("change", (event) => { this._exportStatus = event.target.value || "all"; });
    this.shadowRoot.getElementById("export-category")?.addEventListener("change", (event) => { this._exportCategory = event.target.value || "all"; });
    this.shadowRoot.getElementById("export-from")?.addEventListener("change", (event) => { this._exportFrom = event.target.value || ""; });
    this.shadowRoot.getElementById("export-to")?.addEventListener("change", (event) => { this._exportTo = event.target.value || ""; });
    this.shadowRoot.getElementById("export-trend")?.addEventListener("change", (event) => { this._exportTrend = event.target.value || "both"; });
    ...
    this.shadowRoot.getElementById("toggle-current-bills")?.addEventListener("click", () => {
    ...
    this.shadowRoot.getElementById("all-bills-status")?.addEventListener("change", (event) => {
      this._allBillsStatus = ["all", "unpaid", "paid"].includes(event.target.value) ? event.target.value : "all";
      this._allBillsPage = 1;
      this._render();
    });
  ...
  async _exportData() {
    if (!this._hass || this._transferBusy) return;
    const fromMonth = this.shadowRoot.getElementById("export-from")?.value || this._exportFrom || "";
    const toMonth = this.shadowRoot.getElementById("export-to")?.value || this._exportTo || "";
  ...
  async _submit(event) {
    ...
    const paymentDate = this.shadowRoot.getElementById("payment-date")?.value || "";
    const dueDate = this.shadowRoot.getElementById("due-date")?.value || "";
    const provider = this.shadowRoot.getElementById("provider")?.value.trim() || "";
    const contract = this.shadowRoot.getElementById("contract")?.value.trim() || "";
    const consumptionRaw = this.shadowRoot.getElementById("consumption")?.value ?? "";
    const consumption = String(consumptionRaw).trim() === "" ? undefined : Number(consumptionRaw);
...
if (!customElements.get("bill-tracker-card-impl")) customElements.define("bill-tracker-card-impl", BillTrackerCard);
if (!customElements.get("bill-tracker-card-editor-impl")) customElements.define("bill-tracker-card-editor-impl", BillTrackerCardEditor);
